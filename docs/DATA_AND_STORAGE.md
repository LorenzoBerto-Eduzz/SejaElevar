...
Data Health / Pendencias is the app's live validation safety layer. It should read the active on-use `DadosElevar` workbook directly, not stale UI state, and report broken, incomplete, suspicious, or document-dangerous relationships across worksheets. The implementation lives in `project/src/shared/data/dataHealth.ts`, shares structural dependency rules with `project/src/shared/data/dependencyInspector.ts`, and currently exposes a diagnostic guard button/popup in the global sidebar controls through `project/src/shared/ui/DataHealthButton.tsx`. This is not final coworker-facing visual design; the important product foundation is the modular validation engine. See `docs/DATA_HEALTH.md` for the durable guard model.

Current checks include worksheet/header read failures, duplicate catalog values, blank required values, invalid Disciplina workloads and Horas Aplicadas minutes, invalid Presenca status, Aprendizes without or pointing to missing Arcos/Turmas, missing Plano de Ensino rows for Aprendizes with Arco, Aulas without coverage Disciplinas, broken Aula-to-Disciplina coverage links, Cronograma events without Aula/Turma or with missing/incomplete linked records, Presencas marked `Presente` without expected `Horas Aplicadas`, `Horas Aplicadas` pointing at no active Presenca, missing or stale `Plano Progresso` cache rows, duplicate/colliding stable IDs, and orphaned cross-sheet references. Broken live references are blocking; detached links inside intentionally frozen historical snapshots are warnings, not automatic rewrites. Aprendiz without Turma is currently operational info, while Aprendiz without Arco is at least warning-level because it prevents academic plan/progress tracking. Presencas/Horas pointing to a missing Aprendiz should be treated as a serious integrity problem unless the official privacy purge removed those rows too. This list must stay modular and expand with deletion/document rules.

Aprendiz privacy deletion is a separate, irreversible storage transaction:

- The UI reads the active workbook and shows exact counts for the Aprendiz row plus linked `Plano de Ensino`, `Presencas`, `Horas Aplicadas`, and `Plano Progresso` rows.
- Confirmation requires typing the exact Aprendiz name. The resulting workbook removes only rows owned by the stable `Aprendiz ID`; other people and shared catalog/schedule data remain.
- `PUT /api/privacy/aprendiz-purge` replaces the active workbook with the sanitized copy, removes backups/global checkpoints/generated indexes and known per-Aprendiz document folders, disables recovery for the purged state, and invalidates browser undo/action history through `historyResetId`.
- `dados/sistema/privacy-state.json` stores no name or raw personal ID. It keeps the latest history-reset token and SHA-256 hashes of purged stable IDs. Global import validation rejects an old workbook that would reintroduce one of those identities.
- A unified workbook import that omits any currently registered Aprendiz is blocked. Manual spreadsheet row deletion is not accepted as privacy compliance because it can leave linked history/documents behind.
- Automated coverage lives in `privacyIntegrity.test.ts`; `npm test` covers pure transforms/integrity transitions and `npm run test:privacy-provider` exercises the real launcher/provider in an isolated temporary workspace.
...
The global controls should avoid transient disabled states during valid data operations. They currently live in the sidebar bottom and collapsed-sidebar mini-bars rather than page headers, but their data behavior is still global. Once the app has confirmed an active `DadosElevar` workbook, `Exportar` should remain enabled unless that workbook is truly removed or missing. `Recuperar Dados` should be enabled whenever there is a recoverable checkpoint workbook under `dados/checkpoints/`, and should not lock/unlock merely because the user switched tabs. During recovery swaps, undo/redo, or normal refreshes, `Recuperar Dados` should not flash disabled just because an intermediate refresh briefly sees stale provider state; successful recovery responses should include refreshed global recovery metadata, and the frontend should preserve or normalize the active toolbar state while the restored workbook/checkpoint metadata settles.

UI tabs do not need to match workbook worksheets 1:1. For example, the `Turmas` page can read both the `Turmas` and `Aprendizes` worksheets to show apprentices grouped by turma, and the `Arcos` page can inspect the Disciplinas derived from imported official Ementa PDFs.
...
- The root of `dados/` should contain only the current timestamped on-use workbook file, `.gitkeep`, and the intentional folders `checkpoints/`, `ementas/`, and `sistema/`. Old workbook files that are not active should be deleted by the provider after import/save/patch/recovery; historical recovery workbooks live under `dados/checkpoints/`, not as loose root `.xlsx` backups. Imported official Ementa PDFs live under `dados/ementas/`, and app metadata/control JSON belongs under `dados/sistema/`.
...
Durable data pipeline rule: the active `DadosElevar` workbook is the current operational source that app pages work from. Internal/generated data such as `dados/sistema/data-index.json` adapts itself to that workbook: it indexes, normalizes, links, and orchestrates data for UI/search/documents, but it is not an independent replacement for the workbook. When the app gains document-import features, those documents should feed/update the workbook and then refresh the internal/generated data. For example, importing an official Ementa PDF should parse the document, store a copy under `dados/ementas/`, write/update the `Arcos` and `Disciplinas` sheets in the active workbook, refresh the generated index, and only then display the Arcos tab from that workbook/internal state. Do not create detached feature data from documents, and do not treat a document import as a substitute for the active `DadosElevar` workspace unless the user explicitly redesigns the storage model.

Arcos/Disciplinas are live rotational app data. New official Ementas can be imported after the app already has Aprendizes, Turmas, Aulas, Cronograma entries, or open tabs. The import path must update the workbook, refresh generated index entities, and notify mounted UI immediately so linked dropdowns and coverage pickers use the new data without waiting for a restart or tab reload. The durable dependency chain is: `Ementa PDF -> Arcos/Disciplinas sheets -> Aprendiz Arco + Aula coverage -> Cronograma event -> Presenca -> Plano de Ensino progress/documents`.
...
Aprendizes, Turmas, Arcos, Disciplinas, Aulas, Aulas Disciplinas, and Cronograma have separate generated-index entity definitions. `Disciplinas` is an internal/indexed concept managed through the Arcos flow for now, not a main sidebar tab. `Aulas Disciplinas` is the link/index entity that tells which Disciplinas a reusable Aula covers. Future tabs such as Empresas and Documentos should add their own entities instead of mixing data into existing entities.
...
The Aulas coverage UI is now the next active foundation, not a future blank slate. Coverage rows are edited through `Aulas Disciplinas`: the UI adds/selects existing Disciplinas from the current imported Arcos/Disciplinas catalog, displays them as `Modulo  |  Disciplina`, excludes already-selected coverage options for that Aula, and keeps the workbook/index as the source. Selecting or changing a coverage item must update `Aulas Disciplinas`, refresh generated entities, and remain in sync if the user imports another Ementa while Aulas is mounted. Shared `Inicial`/`Basico` coverage uses the module label; `Especifico` coverage should use the relevant Arco context rather than a generic "Especifico" label when it helps the user choose. Current visual direction for the Aulas page is: the Aula row stays on the first line, `Adicionar Disciplina` stays to its right, selected coverage chips render below in rows of four equal-width items, and the fifth wraps to the next row. Long clipped coverage labels show `...`; only clipped labels expand on hover, and that expansion should grow downward as an overlay without moving the chip, first text line, or neighboring items. Coverage deletion is intentionally not shown as a normal chip affordance for now; deletion can return later in a different, cleaner interaction.
...
Before investing deeply in `Documentos`, the data foundation should be made document-ready. That means each document value should have a clear source: direct workbook field, generated index field, derived relationship, historical attendance/progress record, or user-filled document parameter. The app should prefer generated/indexed records over scraping visible UI, and document generation should read from stable IDs/relationships so future templates can reliably reference Aprendiz, Turma, Arco, Disciplina, Aula, Cronograma, Presenca, and Plano de Ensino data.
...
- `Arco de Aprendizagem` on an Aprendiz is now a dropdown sourced from imported/registered `Arcos`.
...
Recovery reason text belongs to the checkpoint entry, not merely to a mirrored top-level control value. At app startup, a `before_edit` checkpoint from a previous launched app session must be aged to `before_session_edit` so the modal says `antes da última sessão com edições` instead of incorrectly saying `nesta sessão`. Legacy `after_recovery` metadata should be normalized to the current `before_recovery` meaning; the product copy should not invite the user to recover to "after" a recovery because the active mechanism stores the state from before the last recovery as the new reversible checkpoint.
...
Action history is truthful only while its file dependencies still exist. Global import/recovery undo entries depend on checkpoint ids under `dados/checkpoints/`; normal edit/register/delete entries depend on there being an active on-use workbook. Before running undo/redo, `project/src/shared/undo/globalUndo.ts` validates global import/recovery checkpoint dependencies against `/api/recovery`, which also prunes missing checkpoint metadata. If the next checkpoint-dependent action points at a pruned or manually deleted backup, the app must not try to cross that boundary. It discards the unsafe undo/redo path and records the blue diagnostic line `Histórico cortado: backup usado por recuperação/importação não existe mais.`. If the active on-use workbook is missing, normal edit undo is paused instead of applied, with `Histórico pausado: arquivo de dados em uso não está disponível.`. This means undo can cross recoveries/imports only as far as the required checkpoint files still exist; that is intentional and safer than replaying old edits onto the wrong workbook state.

The dev app has a non-release action-history overlay in `project/src/shared/actionLog/`. It is toggled from the sidebar `Histórico` button / hidden-sidebar action mini-bar, is visually click-through, lists newest actions at the top, colors done actions green, undone actions red, undone redo-path actions discarded by a new action yellow, and explanatory cut/reason lines blue. It is a diagnostic tool for understanding the global undo/checkpoint flow and should not become coworker-facing product UI unless explicitly redesigned.
...
- `dados/`: the active timestamped `DadosElevar` workbook plus the intentional `checkpoints/`, `ementas/`, and `sistema/` folders.
- `dados/ementas/`: stored copies of imported official Ementa PDFs used to derive Arcos and Disciplinas.
