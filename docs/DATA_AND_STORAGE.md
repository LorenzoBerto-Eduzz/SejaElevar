...
Current planning checkpoint: pause deeper Turmas timetable/Aula-instance work until the upstream academic catalog exists. The first read-only `Arcos` worksheet/UI foundation now exists, reading `Arcos` and `Disciplinas` from the active unified workbook. The next foundation should make that Arcos flow editable so each Arco's plano de ensino/ementa Disciplinas can be created and maintained. After that, `Aulas` can select which Disciplinas they cover, and only then should Cronograma block selection/discipline display be completed.
