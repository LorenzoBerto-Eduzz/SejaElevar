...
Data Health / Pendencias is the app's live validation safety layer. It should read the active on-use `DadosElevar` workbook directly, not stale UI state, and report broken, incomplete, suspicious, or document-dangerous relationships across worksheets. The current v1 implementation lives in `project/src/shared/data/dataHealth.ts` with a temporary top-left diagnostic popup in `project/src/shared/ui/DataHealthButton.tsx`. It is intentionally not the final coworker-facing visual design; it exists so the user can see whether the app is catching real issues while the product grows. Future work should keep adding rules as new flows/document requirements appear, and the same validation rules should be reused for contextual red warning toasts at the action point.

Current checks include Aprendizes without Arco/Turma, Aprendizes pointing to missing Arcos/Turmas, missing Plano de Ensino rows for Aprendizes with Arco, Aulas without coverage Disciplinas, Cronograma events without Aula/Turma or with missing linked records, Presencas marked `Presente` without expected `Horas Aplicadas`, `Horas Aplicadas` pointing at no active Presenca, missing `Plano Progresso` cache rows, duplicate/colliding stable IDs, and orphaned cross-sheet references. `project/src/shared/data/dependencyInspector.ts` is the shared structural authority used by Data Health and import validation. Broken live references are blocking; detached links inside intentionally frozen historical snapshots are warnings, not automatic rewrites. This list must stay modular and expand with deletion/document rules.
