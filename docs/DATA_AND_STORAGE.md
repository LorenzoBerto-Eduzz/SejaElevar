...
Current planning checkpoint: pause deeper Turmas timetable/Aula-instance work until the upstream academic catalog exists. The `Arcos` worksheet/UI foundation now reads `Arcos` and `Disciplinas` from the active unified workbook, but the product decision changed from manual Arco/Disciplina editing to official Ementa-driven creation. `Adicionar Arco` imports an official Ementa PDF, stores a copy under `dados/ementas/`, parses the current Elevar PDF mold, writes/updates `Arcos` and `Disciplinas`, and displays that academic catalog read-only. The parser should tolerate official workload text written either as `24 horas` or compact `24h`, but it must reject a partially parsed Ementa that has an Arco name and no `Específico` discipline rows. The Arcos visual is currently a horizontal academic catalog: Arcos are columns, module buttons span the full content width and expand for all Arcos at once, default module state has `Específico` open and `Inicial`/`Básico` closed, and importing another Ementa preserves the user's current module toggles. Disciplina bubbles reserve a larger two-line name area plus a fixed right rail for workload and the future Aulas inspection button. `Aprendizes.Arco de Aprendizagem` is now a dropdown/reference field sourced from the imported `Arcos` worksheet and should refresh immediately when new Ementas add Arcos. After that, `Aulas` can select which Disciplinas they cover, and only then should Cronograma block selection/discipline display be completed.
