...
- Global undo now lives in `project/src/shared/undo/globalUndo.ts`. Aprendizes and Turmas register page controllers there, and `App.tsx` owns the app-level `Ctrl+Z` capture/navigation. Undo entries are global rather than strictly page-local: if an action was made from another tab, undo switches back to the origin tab and runs that page's undo handler so files, visible rows, item popups, and generated indexes stay aligned. Boundary undo helpers (`getGlobalUndoBoundarySnapshot`, `pushGlobalBoundaryUndoEntry`, and `replaceGlobalUndoStack`) support import/recovery style app-state changes without letting old detailed undo history leak into a new imported timeline.
