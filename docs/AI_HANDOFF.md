...
Startup color behavior: `project/index.html` includes a small early boot script/style that reads `sejaelevar.darkMode` and `sejaelevar.settings` from `localStorage` before React loads, then applies the matching dark/light class and background to the raw document. This reduces visible white/black/white flashes while keeping launcher startup reliable. Do not reintroduce the attempted hidden-window `/api/app/ready` reveal handshake without careful real-WebView testing; it caused the exe to start as an invisible stuck process when the ready signal did not reveal the form. The launcher currently uses the reliable `NavigationCompleted` reveal path.
