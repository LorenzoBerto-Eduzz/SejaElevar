...
- Global checkpoints capture the whole app data state, currently active Aprendizes and Turmas workbooks together. The first app import creates a disabled original checkpoint until a distinct edit exists. Importing any workbook while app data already exists captures the previous whole-app state as a recoverable `before_import` checkpoint. The provider keeps up to three checkpoint filesets under `dados/checkpoints/`, newest first. Repeated sequential imports in the same launched app session amend the same import checkpoint/action; a new edit, recovery, or launched session breaks that sequence.
...
- Current normal recovery descriptions identify: first imported app-data state, state before the last import, state before the last recovery, state before edits in the current session, or state before the last session with edits. The UI should avoid saying that recovery belongs to only Aprendizes, Turmas, or a single file; it is a `Dados` checkpoint.
...
- Global undo now lives in `project/src/shared/undo/globalUndo.ts`. Aprendizes and Turmas register page controllers there, and `App.tsx` owns the app-level `Ctrl+Z`/redo capture/navigation. Undo entries are global rather than strictly page-local: if an action was made from another tab, undo/redo switches back to the origin tab and runs that page's handler so files, visible rows, item popups, and generated indexes stay aligned. Boundary undo helpers (`getGlobalUndoBoundarySnapshot`, `pushGlobalBoundaryUndoEntry`, and `replaceGlobalUndoStack`) support import checkpoints, while recovery is tracked as a normal reversible global action on top of the current history. The dev-only action history overlay lives in `project/src/shared/actionLog/`; it shows newest actions first, green for done, red for undone, yellow for undone actions whose redo path was discarded by a new action, and blue only for hard history cuts such as the 200-action limit. Starting a new session does not add a blue line because undo/recovery history is intentionally preserved.
- The dev action-history log format should stay compact: `Tab | ação | item-ref | Valor: "antigo" -> "novo"`. Examples: `Aprendizes | editado | apr#3 | Turma: "vazio" -> "Quarta Tarde"`, `Turmas | descadastrado | apr#3`, `Turmas | importado | Turmas_153022100626.xlsx`. Do not wrap the tab in brackets and do not use a long student name as the primary identifier. Current `apr#n` refs are log-friendly row-based refs, matching the current generated index idea (`aprendizes:n`), not true permanent universal IDs. A future stable app-managed ID system should replace these refs when relationships/search/doc generation need identity that survives row moves, blank names, imports, or deletes.
