...
- In normal Aprendizes view mode, clicking a row opens the current student/apprentice item popup shell and keeps that row highlighted. The popup closes only with its `X`; clicking another row swaps the selected item. Its preferred size is exposed as dev-only settings sliders and currently defaults to roughly 804px by 591px. It is anchored 20px from the table bottom/right when possible, then shrinks to stay inside the usable table area bounded by the sticky first column, header row, right edge, and bottom edge margins. The item popup currently has a real editable `Nome` field wired through the same edit/save/undo path as table cells, plus seven inert `Nome` template layers used only to evaluate future detail-field spacing. Settings/search popups must layer above item popups. The popup positioning work is intentionally debounced during table/sidebar resizing so the sidebar hide/show animation stays smooth while the popup is open.
