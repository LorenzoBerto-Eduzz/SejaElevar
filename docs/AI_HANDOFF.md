...
- Importing a workbook whose headers do not match the current tool schema shows a bottom-right red toast for 3 seconds: `Arquivo importado não possui os valores necessários`. The toast is one line, floats above the app chrome/sidebar, and has a red bottom timer bar that drains right-to-left. Light mode uses a vivid red; dark mode uses a medium darker red.
