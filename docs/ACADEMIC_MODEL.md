...
Shared implementation rule: attendance status reading, draft/selection creation, event snapshot creation, live Aula resolution for unconfirmed blocks, edit/move/delete guards, row construction/mutation, undo/redo row insertion/removal, and active-workbook persistence belong in shared Cronograma modules. `Calendário` and `Turmas` should call those shared rules rather than maintaining equivalent local copies. Cronograma workbook writes are serialized through one shared queue so both mounted views cannot write concurrently through independent paths. Context-specific rendering, collision rules, navigation, undo origin, and user feedback may remain in each page, but the meaning and stored shape of an event must stay identical.
