...
- A user request for `memcheck` means: thoroughly update the appropriate long-term memory docs with the distilled outcome of the recent discussion. It does not mean commit or push.
- A user request for `gitcheck` means: perform `memcheck`, inspect the current worktree, run relevant checks when practical, verify the Git identity guard, stage intended files, commit, and push to the configured remote unless the user says not to.
- Before creating a commit or pushing, check `.git-identity`, `git config user.email`, and `git config core.hooksPath`. The local Git email must match `lorenzo.berto@eduzz.com`, and hooks must use `.githooks`.
- Keep `git config core.hooksPath .githooks` enabled in every clone so the tracked hooks block accidental commits or pushes from another account email. On a newly pulled clone, run `git config core.hooksPath .githooks` once if needed. `git config user.name` may vary by device.
- If the identity guard blocks a commit/push, do not bypass it. Fix the local Git config or ask the user before proceeding.
- During `gitcheck`, do not stage/push `project/dev/SejaElevar.exe` just because a normal build touched the dev package. The launcher is a large Git LFS file and can make pushes stall for a long time. Normal `gitcheck` should prefer committing source/docs/dev HTML/assets and leaving the exe out unless the user explicitly asks to distribute a rebuilt binary through Git. If launcher/provider source changed, commit that source and let other machines rebuild locally with `npm --prefix project run build:single`; only include the rebuilt exe when cross-device runnable-dev handoff is more important than avoiding the LFS upload.
- `gitcheck` commit messages should let the user understand the commit later from `git log`/`git show` without needing the chat. Mention the main implementation, docs/workflow updates, tuning/config changes, and notable file/structure changes when they are part of the commit.
