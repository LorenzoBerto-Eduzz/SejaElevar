...
- The first data workflow uses a real student/apprentice `.xlsx` spreadsheet as the active source. Importing asks only for the source `.xlsx`; the local provider copies it directly into `dados/` as `Aprendizes_hhmmssddmmyy.xlsx`. The provider tracks the current on-use workbook, one backup workbook, and the backup reason in `dados/controle.json`. Real saved edits write a fresh timestamped on-use workbook so the filename reflects the latest update. Import backups are preserved through later edits; backup replacement on edit is only allowed for missing backups, normal previous-session backups, or already-restored backups. `Recuperar Dados` restores by copying the tracked backup into a fresh on-use workbook while keeping the backup intact.
