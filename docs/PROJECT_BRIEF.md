...
- Primary language/stack: Planned initial stack is Vite + React + TypeScript for the browser UI, plus a small local Node service/backend for file access and document generation. No app scaffold exists yet.
...
- The app should run as a browser-accessed local web app during early development.
...
- Design the storage boundary so future adapters can target Google Drive folders, Google Sheets/Drive APIs, a hosted database, or another backend without rewriting every UI feature.
...
1. Initialize Git and create the first project-frame checkpoint once the user asks.
2. Scaffold the first local web app using Vite + React + TypeScript plus a small local Node service.
...
- This repo is not yet initialized as Git at the time this brief was first adapted.
